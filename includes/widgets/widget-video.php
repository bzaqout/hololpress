...
add_action( 'widgets_init', 'holol_video_widget_box' );
function holol_video_widget_box() {
	register_widget( 'holol_video_widget' );
}
class holol_video_widget extends WP_Widget {

	function holol_video_widget() {
		$widget_ops 	= array( 'classname' => 'video-widget', 'description' => ''  );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'video-widget' );
		parent::__construct( 'video-widget', __( 'فيديو' , 'tie') , $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="video-container">';
			...
			if (!empty( $instance['embed_code'] ) ){
				...
				$video_output = $wp_embed->run_shortcode('[embed width="340" height="220"]'.$video_url.'[/embed]');
