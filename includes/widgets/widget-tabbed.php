...
class holol_widget_tabs extends WP_Widget {
	function holol_widget_tabs() {
		...
		parent::__construct( 'widget_tabs', __( 'تبويبات' , 'tie') , $widget_ops );
	}
	...
	<div class="widget" id="tabbed-widget">
			...
			<div class="widget-top">
				...
				<?php
					...
					foreach ( $tabs_order_array as $tab ){
						...
						if( $tab == 'p' )
							echo '<li class="tabs"><a href="#tab1">'. __( 'الأشهر' ) .'</a></li>';	
							
						if( $tab == 'r' )
							echo '<li class="tabs"><a href="#tab2">'. __( 'الأخيرة' ) .'</a></li>';	
														
						if( $tab == 't' )
							echo '<li class="tabs"><a href="#tab4">'. __( 'وسوم' ) .'</a></li>';
					}
				?>
				</ul>
			</div>
		...
		foreach ( $tabs_order_array as $tab ){
			if(++$i==1)
				...
				$active = '';
			if( $tab == 'p' ) : ?>
			<div id="tab1" class="tabs-wrap <?=$active?>">
				<ul>
					<?php 
			...
			if( $tab == 'r' ) : ?>
			<div id="tab2" class="tabs-wrap <?=$active?>">
				<ul>
			...
			if( $tab == 't' ) : ?>
			<div id="tab4" class="tabs-wrap tagcloud <?=$active?>">
	...
	function form( $instance ) {
		...
		<div id="<?php echo $widget_id ?>-tabs">
			<p>
				...
				<?php if( $id[2] == '__i__' ) echo '<p style="background-color: #fefbed;padding: 5px;color: #9F6000;border: 1px solid #f1e7bc;" class"holol_message_hint">'. __( "click Save button to be able to change the order of tabs ." , "tie").'</p>'?>
				...
				<?php
					...
					foreach ( $tabs_order_array as $tab ){
						...
						if( $tab == 'p' )
							echo '<li data-tab="p"> '. __( "Popular" ) .' </li>';	
