<?php
add_action( 'widgets_init', 'holol_comments_avatar_widget' );
function holol_comments_avatar_widget() {
	register_widget( 'holol_comments_avatar' );
}
class holol_comments_avatar extends WP_Widget {

	function holol_comments_avatar() {
		$widget_ops 	= array( 'classname' => 'comments-avatar' );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'comments_avatar-widget' );
		parent::__construct( 'comments_avatar-widget', __( 'آخر التعليقات مع صورة' , 'tie' ) , $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
			<ul>	
		<?php holol_most_commented( $no_of_comments , $avatar_size); ?>
		</ul>
	<?php 
		echo $after_widget;
	}
