<?php
add_action( 'widgets_init', 'holol_timeline_posts_widget' );
function holol_timeline_posts_widget() {
	register_widget( 'holol_timeline_widget' );
}
class holol_timeline_widget extends WP_Widget {

	function holol_timeline_widget() {
		$widget_ops 	= array( 'classname' => 'timeline-posts' );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'timeline-posts-widget' );
		parent::__construct( 'timeline-posts-widget', __( 'جدول زمني' , 'tie') , $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
				<ul>
					<?php holol_last_posts_cat_timeline($no_of_posts , $cats_id)?>	
				</ul>
		<div class="clear"></div>
	<?php 
		echo $after_widget;
