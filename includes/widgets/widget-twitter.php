...
add_action( 'widgets_init', 'holol_latest_tweet_widget' );
function holol_latest_tweet_widget() {
	register_widget( 'holol_Latest_Tweets' );
}
class holol_Latest_Tweets extends WP_Widget {

	function holol_Latest_Tweets() {
		$widget_ops 	= array( 'classname' => 'twitter-widget'  );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'latest_tweets_widget' );
		parent::__construct( 'latest_tweets_widget', __( 'تويتر' , 'tie') , $widget_ops, $control_ops );
	}
	...
	if( !empty($twitter_username) && !empty($consumer_key) && !empty($consumer_secret) ){
	    ...
	    $token 			= get_option( 'holol_TwitterToken'.$widget_id );
		...
		if( empty( $twitter_data ) ){
			...
			if( !$token ) {
				...
				if($keys) {
					...
					update_option( 'holol_TwitterToken'.$widget_id , $keys->access_token);
...
<?php			}
	...
	function update( $new_instance, $old_instance ) {
		...
		delete_option(    'holol_TwitterToken'.$widget_id );
