<?php
add_action( 'widgets_init', 'holol_social_widget_box' );
function holol_social_widget_box() {
	register_widget( 'holol_social_widget' );
}
class holol_social_widget extends WP_Widget {

	function holol_social_widget() {
		$widget_ops 	= array( 'classname' => 'social-icons-widget' );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'social' );
		parent::__construct( 'social', __( 'أيقونات التواصل' , 'tie') , $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( !$tran_bg ){
			...
			echo $title ; 
			echo $after_title;
				get_social_icon( $newtap, $colored, 'ttip-none' );
			echo $after_widget;
		}
		else { ?>
			<div class="widget social-icons-widget">
			<?php get_social_icon( $newtap , $colored, 'ttip-none' ); ?>
			</div>
		<?php }			
	}
