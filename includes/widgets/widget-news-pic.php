<?php
add_action( 'widgets_init', 'holol_news_pic_widget' );
function holol_news_pic_widget() {
	register_widget( 'holol_news_pic' );
}
class holol_news_pic extends WP_Widget {

	function holol_news_pic() {
		$widget_ops 	= array( 'classname' => 'news-pic' );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'news-pic-widget' );
		parent::__construct( 'news-pic-widget',__( 'آخبار في صور' , 'tie'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
				<?php holol_last_news_pic($posts_order , $no_of_posts , $cats_id)?>	
		<div class="clear"></div>
	<?php 
		echo $after_widget;
	}
