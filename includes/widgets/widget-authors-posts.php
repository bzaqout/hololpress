<?php
add_action( 'widgets_init', 'holol_authors_posts_widget' );
function holol_authors_posts_widget() {
	register_widget( 'holol_authors_posts' );
}
class holol_authors_posts extends WP_Widget {

	function holol_authors_posts() {
		$widget_ops 	= array( 'classname' => 'authors-posts' );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'authors-posts-widget' );
		parent::__construct( 'authors-posts-widget', __( "تدوينات الناشر" , 'tie' ) , $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
				<ul>
					<?php holol_last_posts_cat_authors($no_of_posts , $thumb , $cats_id)?>	
				</ul>
		<div class="clear"></div>
	<?php 
		echo $after_widget;
