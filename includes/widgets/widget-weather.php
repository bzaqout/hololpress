...
// THE LOGIC
function holol_weather_logic( $atts ){
	...
	if( !$location ) { return holol_weather_error(); }
	...
	// TRANSIENT NAME
	$weather_transient_name 		= 'holol_' . $city_name_slug . "_" . strtolower($units_display) . '_' . $locale;
	...
	if( get_transient( $weather_transient_name ) ){
		...
		if( is_wp_error( $now_ping_get ) ){
			return holol_weather_error( $now_ping_get->get_error_message()  ); 
		}
	
		$city_data = json_decode( $now_ping_get['body'] );
		
		if( isset($city_data->cod) AND $city_data->cod == 404 ){
			return holol_weather_error( $city_data->message ); 
		}else
		{
		...
		if( $days_to_show != "hide" )
		{
			...
			if( is_wp_error( $forecast_ping_get ) ){
				return holol_weather_error( $forecast_ping_get->get_error_message()  ); 
			}	
			
			$forecast_data = json_decode( $forecast_ping_get['body'] );
			
			if( isset($forecast_data->cod) AND $forecast_data->cod == 404 ){
				return holol_weather_error( $forecast_data->message ); 
			}else{
	...
	// NO WEATHER
	if( !$weather_data OR !isset($weather_data['now'])) { return holol_weather_error(); }
...
// RETURN ERROR
function holol_weather_error( $msg = false )
{
	if(!$msg) $msg = __('No weather information available', 'tie');
	return apply_filters( 'holol_weather_error', "<!-- TIE WEATHER ERROR: " . $msg . " -->" );
}
...
class TIE_WeatherWidget extends WP_Widget 
{
    ...
    {	
		...
		echo holol_weather_logic( array( 'location' => $location, 'api_key' => $api_key, 'units' => $units, 'forecast_days' => $forecast_days ));
		echo $after_widget;
    }
...
add_action( 'widgets_init', 'holol_weather_widget' );
function holol_weather_widget() {
