<?php
add_action( 'widgets_init', 'holol_categort_posts_widget' );
function holol_categort_posts_widget() {
	register_widget( 'holol_categort_posts' );
}
class holol_categort_posts extends WP_Widget {

	function holol_categort_posts() {
		$widget_ops 	= array( 'classname' => 'categort-posts' );
		$control_ops 	= array( 'width' => 250, 'height' => 350, 'id_base' => 'categort-posts-widget' );
		parent::__construct( 'categort-posts-widget', __( "تدوينات التصنيف" , 'tie' ) , $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
				<ul>
					<?php holol_last_posts_cat($no_of_posts , $thumb , $cats_id)?>	
				</ul>
		<div class="clear"></div>
	<?php 
		echo $after_widget;
